...
//           Else go in the while loop. If there is a capital letter it changes to 
//           lower case. Then it checks to see if there are any puctuations and 
//           deletes them. If the word is not in the linked list add it in 
//           ascending order. Close the file.   
void buildLL(LL & LinkedList)
{
  //char letter;
  string word;
  ifstream fin;
...
//           delete it.   
void removeBadWords(LL & link)
{
  string word;
  ifstream inFile;
  inFile.open("badWords.txt");

  while (inFile >> word)//{} why did it still work? if stmt Technically 1 line of code?
  if (link.search(word))
...
void outputToFile(LL & Link_List)
{
  string word;
  string fileName;
  LL templA;
  LL templB;
  ofstream outFile;
  char ourLet = '?';//dummy(prof slott said make a dummy variable)
  char previous;//

  while(!Link_List.isEmpty())
  {
      previous = ourLet;
      word = Link_List.deleteFront();//deleteFront el_t string.
