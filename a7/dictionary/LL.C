...
//PARAMS: New element e of type el_t.
//ALGORITHM: Create two pointers pre and del. If the linked list is empty return.
...
void LL::deleteNode(el_t e)
{
  ...
  else
  {
    node* pre;
    node* del;   
    for(pre = front, del = front-> next; del!=NULL &&del-> element != e; pre=del, del = del->next)// general case
  ;
